...
class InferNode(Node):
    def __init__(self):
        ...
        self.filtered_bboxes_nms = torch.zeros(0, 6)
        self.filtered_bboxes_tensor = torch.zeros(0, 7)
        self.filtered_scores = torch.zeros(0)
        self.filtered_labels = torch.zeros(0)
    ...
    def listener_callback(self, msg):
        ...
        self.logger.info("Process time: {:.2f} ms".format(elapsed_time_ms))
        ...
        elapsed_time_ms = (end_time - start_time) * 1000
        self.logger.info("Inference time: {:.2f} ms".format(elapsed_time_ms))

        # applied to tr3d repo
        bboxes = model_result[0]['boxes_3d']
        scores = model_result[0]['scores_3d']
        labels = model_result[0]['labels_3d']
        #bboxes = [x['boxes_3d'] for x in model_result]
        #scores = [x['scores_3d'] for x in model_result]
        #scores = torch.cat(scores)
        #labels = [x['labels_3d'] for x in model_result]
        #labels = torch.cat(labels)
        # applied to mmdetection3d
    ...
    def detections_callback(self):
        ...
        self.draw_bbox(self.filtered_bboxes_tensor, self.filtered_labels.numpy(), self.filtered_scores.numpy())
    ...
    def draw_bbox(self, bboxes, labels, scores, timestamp=None):
        ...
        if len(bboxes) > 0:
            for ind in range(len(bboxes)):
                ...
                # ref: https://github.com/ros-perception/vision_msgs/blob/foxy/vision_msgs/msg/ObjectHypothesisWithPose.msg
                object_hypothesis.id = self.class_names[label]
                object_hypothesis.score = score.item()
                det3d.results.append(object_hypothesis)
                
                det3d_array.detections.append(det3d)
                
                self.logger.info(f"Found class_id: {self.class_names[label]}, score: {score.item()}")
